...
import { skillSchema, idSchema, idWithUpdateSchema } from "@/lib/validation";

const updateSchema = idWithUpdateSchema(skillSchema);
...
export async function PUT(request: Request) {
  try {
    ...
    return NextResponse.json(await prisma.skill.update({ where: { id }, data }));
  } catch {
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
